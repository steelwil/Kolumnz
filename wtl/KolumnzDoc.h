...
//
// KolumnzDoc.h: interface for the KolumnzDoc class.
//
//////////////////////////////////////////////////////////////////////

#if !defined(AFX_KOLUMNZDOC_H__C0702AA3_CB45_11D4_A804_00A0C990A69A__INCLUDED_)
#define AFX_KOLUMNZDOC_H__C0702AA3_CB45_11D4_A804_00A0C990A69A__INCLUDED_

const int cJuniorKolumnz = 0;
const int cClassicKolumnz = 1;
const int cCoolKolumnz = 2;
const int cKolumnzMania = 3;
...
//-----------------------------------------------------------------------------
	CKolumnzDoc() 
	{
		InitializeGameData();
		m_hGameFile = 0;
		m_ToolBar = 1;		// view the tool bar by default
		m_StatusBar = 1;	// view the satus bar by default
	}
//-----------------------------------------------------------------------------
	virtual ~CKolumnzDoc()
	{
...
//-----------------------------------------------------------------------------
	bool InitializeGameData()
	{
		...
		m_Game.m_Penalize = 1;
		strcpy(m_Game.m_Comment, "Standard Kolumnz");
		for (int i=0; i<10; i++)
		{
			...
			m_Game.m_HighScore[i].score = (10-i)*500;
...
//-----------------------------------------------------------------------------
	...
	{
		// TODO: add storing code here
		ATLTRACE("CKolumnzDoc::OpenGameFile\n");
		bool rtc = true;
		if (fn)
		{
			if (m_hGameFile)
			...
			if (INVALID_HANDLE_VALUE == m_hGameFile)	// could not open file, reopen
			{
				// take out of mru
				UpdateMRUList(false);
				OpenStockGameFile(cClassicKolumnz);
			}
			else
			{
				UpdateMRUList(true);
	...
	{
		// TODO: add storing code here
		ATLTRACE("CKolumnzDoc::OpenStockGameFile\n");
		...
		{
		case cJuniorKolumnz:
			strcpy(m_FileName, "juniorkolumnz.gam");
			break;
		case cClassicKolumnz:
			strcpy(m_FileName, "kolumnz.gam");
			break;
		case cCoolKolumnz:
			strcpy(m_FileName, "coolkolumnz.gam");
			break;
		case cKolumnzMania:
			strcpy(m_FileName, "kolumnzmania.gam");
			break;
		}
		...
		{
		case cJuniorKolumnz:
			if (m_Game.m_BlkSize != 2 || m_Game.m_Colors != 5 || m_Game.m_Base != 6)
			{
				InitializeGameData();
			}
			strcpy(m_Game.m_Comment, "Easy Kolumnz");
			m_Game.m_Base = 6;
			m_Game.m_BlkSize = 2;
			m_Game.m_Colors = 5;
			break;
		case cClassicKolumnz:
			if (m_Game.m_BlkSize != 3 || m_Game.m_Colors != 6 || m_Game.m_Base != 6)
			{
				InitializeGameData();
			}
			strcpy(m_Game.m_Comment, "Classic Kolumnz");
			break;
		case cCoolKolumnz:
			if (m_Game.m_BlkSize != 3 || m_Game.m_Colors != 6 || m_Game.m_Base != 10)
			{
				InitializeGameData();
			}
			strcpy(m_Game.m_Comment, "Cool Kolumnz");
			m_Game.m_Base = 10;
			break;
		case cKolumnzMania:
			if (m_Game.m_BlkSize != 4 || m_Game.m_Colors != 10 || m_Game.m_Base != 14)
			{
				InitializeGameData();
			}
			strcpy(m_Game.m_Comment, "KolumnzMania says it all!");
			m_Game.m_BlkSize = 4;
			m_Game.m_Colors = 10;
			m_Game.m_Base = 14;
			break;
	...
	//-----------------------------------------------------------------------------
	bool OpenNewGameFile(const char* fn)
	{
		// TODO: add storing code here
		ATLTRACE("CKolumnzDoc::OpenNewGameFile\n");
		bool rtc = false;
		if (fn)
		{
			if (m_hGameFile)
...
//-----------------------------------------------------------------------------
	bool UpdateMRUList(bool found)
	{
		// TODO: add storing code here
		ATLTRACE("CKolumnzDoc::UpdateMRUList\n");
		if ((0 != lstrcmpi(m_FileName, "kolumnzmania.gam")) &&
			(0 != lstrcmpi(m_FileName, "Kolumnz.gam")) &&
			(0 != lstrcmpi(m_FileName, "juniorKolumnz.gam")) &&
			(0 != lstrcmpi(m_FileName, "coolKolumnz.gam")))
		{	// do mru list
			int from = 3;
			bool inMRU = false;
			for (int f=0; f<4; f++)
...
//-----------------------------------------------------------------------------
	bool ReadGameFile()
	{
		...
		ATLTRACE("CKolumnzDoc::ReadGameFile\n");
		bool rtc = false;
		DWORD nRead;
		...
		if (INVALID_HANDLE_VALUE != m_hGameFile)
		{
			...
			{
				if (m_Game.m_Base < 2 || m_Game.m_Base > 32 || m_Game.m_PiecePreview > 1)
				{
...
//-----------------------------------------------------------------------------
	bool ReadINIFile(WINDOWPLACEMENT* wp)
	{
		...
		if (GetCurrentDirectory(128, path))
		{
			...
			GetPrivateProfileString("Options", "autoload", "Kolumnz.gam", m_FileName, 128, path);
...
#endif // !defined(AFX_KOLUMNZDOC_H__C0702AA3_CB45_11D4_A804_00A0C990A69A__INCLUDED_)
