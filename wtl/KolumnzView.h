...
//
// KolumnzView.h : interface of the CKolumnzView class
//
/////////////////////////////////////////////////////////////////////////////
...
#include "KolumnzDoc.h"
#include "EnterHighScore.h"

static const int cMAXPLAYERS = 2;

class CKolumnzView : public CWindowImpl<CKolumnzView>
{
public:
	CKolumnzDoc m_Doc;
	CKolumnzEngine m_KolumnzEngine[cMAXPLAYERS];
...
public:
	...
	BEGIN_MSG_MAP(CKolumnzView)
		MESSAGE_RANGE_HANDLER(WM_USER_PAINTGAMEGRID1, WM_USER_PAINTGAMEGRID2, MessageFromKolumnzEngine)
	...
	LRESULT MessageFromKolumnzEngine(UINT uMsg, WPARAM id, LPARAM nRows, BOOL& /*bHandled*/);
...
//-----------------------------------------------------------------------------
LRESULT CKolumnzView::MessageFromKolumnzEngine(UINT uMsg, WPARAM id, LPARAM nRows, BOOL& /*bHandled*/)
{
	...
	case 1:				// game over
		...
		if (m_Doc.IsHighScore(m_KolumnzEngine[p].m_Score))
		{
			CEnterHighScore dlg;
			dlg.DoModal();
			m_Doc.AddHighScore(	dlg.m_Name, dlg.m_Quote, 
								m_KolumnzEngine[p].m_Score,
								m_KolumnzEngine[p].m_TotalBlocksSunk,
								m_KolumnzEngine[p].m_Level);
	...
	case 4:				// cleared lines 
		...
		if (m_Doc.m_Game.m_Penalize && nRows > 1)
			for (int n=0; n<m_Doc.m_NumberOfPlayers; n++)
				if (n != p) m_KolumnzEngine[n].AddRows(nRows);
		break;
	}
	return 0;
}
...
//-----------------------------------------------------------------------------
LRESULT CKolumnzView::OnEraseBkgnd(UINT /*uMsg*/, WPARAM wParam, LPARAM /*lParam*/, BOOL& /*bHandled*/)
{
	CRect rect;
	...
	HBITMAP hOlddcTile = dcTile.SelectBitmap(m_Screens[m_KolumnzEngine[0].m_Level-1].m_hBitmap);
...
//-----------------------------------------------------------------------------
LRESULT CKolumnzView::OnPaint(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
{
...
//-----------------------------------------------------------------------------
void CKolumnzView::GetPlayAreaCoords()
{
	int w = m_RectClient.right - m_RectClient.left;
	int h = m_RectClient.bottom - m_RectClient.top;
...
//-----------------------------------------------------------------------------
void CKolumnzView::PaintGrid(CDC& hDC, int p)
{
	...
	char grid [32][32];
	m_KolumnzEngine[p].GetGrid(grid);
...
//-----------------------------------------------------------------------------
void CKolumnzView::PaintScore(CDC& hDC, int p)
{
	// display the score
	CSize sz;
	...
	if (m_KolumnzEngine[p].m_Score == 0)
	{
		strcpy(out, "         ");
		hDC.GetTextExtent(out, strlen(out), &sz);
		xpos = middle - sz.cx/2;
	...
	wsprintf(out, " %d ", m_KolumnzEngine[p].m_Score);
	hDC.GetTextExtent(out, strlen(out), &sz);
	xpos = middle - sz.cx/2;
	hDC.TextOut(xpos, m_ScoreRect[p].top+25, out);
	...
	wsprintf(out, " %d ", m_KolumnzEngine[p].m_Level);
	hDC.GetTextExtent(out, strlen(out), &sz);
	xpos = middle - sz.cx/2;
	...
	wsprintf(out, " %d ", m_KolumnzEngine[p].m_TotalBlocksSunk);
	hDC.GetTextExtent(out, strlen(out), &sz);
	xpos = middle - sz.cx/2;
...
// display the preview
	...
	int icolor;
	int bs = 10;
	if (m_KolumnzEngine[p].GetPreview())
	{
		rect.left = m_ScoreRect[p].left+26;
		rect.top = m_ScoreRect[p].top+130;
		rect.right = rect.left+4 + (bs+2)*2;
		rect.bottom = rect.top+3 + (bs+2)*4;
		PaintBackBlock(hDC, rect);

		unsigned long pv = m_KolumnzEngine[p].GetPreview();
		for (int r=0; r<m_KolumnzEngine[p].m_PieceLength; r++)
		{
			rect.left = m_ScoreRect[p].left+26+bs+1;
			rect.top = m_ScoreRect[p].top+130+(r+1)*bs+1;
			rect.right = rect.left + bs-2;
			rect.bottom = rect.top + bs-2;
...
//-----------------------------------------------------------------------------
LRESULT CKolumnzView::OnCreate(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
{
...
//-----------------------------------------------------------------------------
LRESULT CKolumnzView::OnKillFocus(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
{
	// TODO : Add Code for message handler. Call DefWindowProc if necessary.
	ATLTRACE("CKolumnzView::OnKillFocus\n");
	::PostMessage(GetParent(), WM_COMMAND, ID_GAME_PAUSEF3, 1);
	return 0;
}

//-----------------------------------------------------------------------------
LRESULT CKolumnzView::OnMouseMove(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& /*bHandled*/)
{
	// TODO : Add Code for message handler. Call DefWindowProc if necessary.
	for (int p=0; p<m_Doc.m_NumberOfPlayers; p++)
	{
...
//-----------------------------------------------------------------------------
inline void CKolumnzView::PaintFlatBlock(CDC& hDC, char block, RECT& rect)
{
...
//-----------------------------------------------------------------------------
inline void CKolumnzView::Paint3DBlock(CDC& hDC, char block, RECT& rect)
{
	CPen pen;
...
//-----------------------------------------------------------------------------
inline void CKolumnzView::PaintPyrimidBlock(CDC& hDC, char block, RECT& rect)
{
	CPoint pts[4];
	CPen pen;
	//hDC.FillSolidRect( rect.left, rect.top, 
...
//-----------------------------------------------------------------------------
inline void CKolumnzView::PaintBackBlock(CDC& hDC, RECT& rect)
{
	hDC.FillSolidRect( rect.left, rect.top, 
